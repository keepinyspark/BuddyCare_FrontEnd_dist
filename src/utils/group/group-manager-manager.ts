import { ChatMessageInterface, Group, ChatChannelEvent } from '@utils/group/dto/group';
...
export class GroupManagerManager extends GroupBaseManager {
	constructor() {
		super();
	...
	handleLoadComplete(): void {
		...
		// this.sendChatMessage({ messageBody: 'hello1', messageType: MessageType.TEXT });
	}
	...
	private handleMessageReceived(e: Event): void {
		...
		if (updateChannel) {
			...
			updateChannel.addMessage(messageDtoList, !isOtherMessage);
	...
	private handleLeaveChannelComplete(e: Event): void {
		const dt = (e as CustomEvent).detail;
		if (this._isDebug) console.warn('chatUserManager.handleLeaveChannelComplete', dt);
	...
	sendChatMessage(dt: { messageBody: string | File; messageType: MessageType; tempUuid: string }): void {
		...
		if (!this.currentChannelIdx || !getUserData().userIdx) return;
		const param = {
			channelToken: this.getChannelToken(this.currentChannelIdx),
		...
		if (dt.messageType === MessageType.TEXT) {
			...
			formData.append('channelIdx', this.currentChannelIdx);
			...
			saveMessageFile(getApiClient(), formData)
				.then(res => {
					...
					const channelToken = this.getChannelToken(this.currentChannelIdx);
					if (channelToken) {
	...
	joinChannel(channelIdx?: string): void {
		...
		if (joinGroup) {
			...
			this._currentGroupIdx = joinGroup.groupIdx;
