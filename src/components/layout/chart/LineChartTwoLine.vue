				...
				if (props.dataType && props.checkList) {
					for (let i = 0; i < props.checkList?.length; i++) {
						...
						if (originData[i] && originData[i].data && originData[i].data.length > 0) {
							lowData = originData[i].data.map((t: any) => t.valueLow);
							highData = originData[i].data.map((t: any) => t.valueHigh);
						}
						series.push({
							type: 'line',
							itemStyle: {
							...
							data: lowData.length === 0 ? [0] : lowData,
						});
						series.push({
							type: 'line',
							itemStyle: {
							...
							data: highData.length === 0 ? [0] : highData,
						});
					}
				}
			}
		...
		const setConfig = () => {
			...
			chartConfig.value = {
				...
				yAxis: {
					...
					min: dataSum > 0 ? null : 0,
					max: dataSum > 0 ? null : 10,
				},
